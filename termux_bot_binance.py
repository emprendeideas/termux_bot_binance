...
async def enviar_botones_async():
    keyboard = [
...
async def enviar_control_ganancia_async():
    keyboard = [
...
async def manejar_botones(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "pause":
        bot_activo = False
        await context.bot.send_message(TELEGRAM_ADMIN_ID, "⏸ Bot pausado")
        await enviar_botones_async()

    elif data == "resume":
        bot_activo = True
        await context.bot.send_message(TELEGRAM_ADMIN_ID, "▶️ Bot reanudado")
        await enviar_botones_async()

    elif data == "close":
        cerrar_manual()
        await context.bot.send_message(TELEGRAM_ADMIN_ID, "🔴 Operación cerrada manualmente")
        await enviar_botones_async()
        ...
        await context.bot.send_message(TELEGRAM_ADMIN_ID, "✅ Se continúa operando")
...
app_telegram.add_handler(CallbackQueryHandler(manejar_botones))        
...
def verificar_ganancia():
    ...
    if ganancia >= nivel_actual:
        ...
        enviar_telegram("📊 Se alcanzó el objetivo mensual.\n¿Desea continuar operando?")
...
if __name__ == "__main__":
    ...
    def run_bot():
        ...
        asyncio.run(enviar_botones_async())
